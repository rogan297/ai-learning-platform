...
mcp-agent-control-plane is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
- **Configuration Management**: Tool-specific configuration via `manifest.yaml`
...
1. **Build Docker Image**:
   ```bash
   kagent mcp build --verbose
   ```

2. **Run in Container**:
   ```bash
   docker run -i mcp-agent-control-plane:latest
   ```

3. **Deploy to Kubernetes**:
   ```bash
   kagent mcp deploy mcp --apply
   ```
...
Configure tools in `manifest.yaml`:
...
kubectl get mcpserver mcp-agent-control-plane
```